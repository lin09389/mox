...
<body>
    ...
    <table>
        ...
        {"".join(f"<tr><td>{r.scenario.scenario_id}</td><td>{r.scenario.name[:30]}</td><td>{r.scenario.evaluation_type.value}</td><td>{'PASS' if r.success else 'FAIL'}</td><td>{r.score:.2f}</td></tr>" for r in results)}
    </table>
</body>
</html>"""
