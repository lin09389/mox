...
class EvaluationWorkflow:
        ...
        for i, case in enumerate(test_cases):
            try:
                ...
                results.append({
                    ...
                    "passed": (case['expected'] == "blocked" and not is_safe) or 
                              (case['expected'] == "allowed" and is_safe),
                })
    ...
    async def _result_aggregation_node(self, state: EvaluationState) -> dict:
        ...
        attack_successes = sum(
            1 for r in results 
            if r.get('expected') == 'allowed' and r.get('actual') == 'blocked'
        )
        defense_blocks = sum(
            1 for r in results 
            if r.get('expected') == 'blocked' and r.get('actual') == 'blocked'
        )
