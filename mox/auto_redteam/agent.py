...
from mox.core import LLMFactory, BaseLLM
from mox.core.llm import Message
from mox.auto_redteam.state import (
    RedTeamTask, StepLog, AgentState, DiscoveredVulnerability, VulnerabilityType
)
...
class RedTeamAgent:
    ...
    def __init__(
        ...
        self.state = RedTeamTask(task_id=task_id, target_model=target_model_name, max_steps=max_steps)
    ...
    async def run(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        self.history.append({"role": "user", "content": f"START TASK. Target Model: {self.target_model_name}. Begin your initial reconnaissance."})
        ...
        while self.state.current_step < self.max_steps and self.state.status == "running":
            ...
            try:
                ...
                if not action_json:
                    ...
                    self.history.append({"role": "user", "content": "ERROR: You must output a valid JSON block for your ACTION. Please try again."})
                ...
                if action_type == "finish":
                    ...
                    self.history.append({"role": "assistant", "content": commander_response})
                    self.history.append({"role": "user", "content": f"ERROR: Unknown action type '{action_type}'."})
                    step_log.state = AgentState.ERROR
                ...
                self.history.append({"role": "user", "content": f"SYSTEM ERROR executing step: {str(e)}"})
                yield self.state.model_dump()
