...
class ManyShotJailbreak(BaseAttack):
    ...
    def _generate_shot_examples(
        ...
        # 添加目标行为示例
        examples.append(ManyShotExample(
            user_prompt=target_behavior,
            assistant_response="[EXPECTED HARMFUL RESPONSE]"
        ))
...
class SkeletonKeyAttack(BaseAttack):
    ...
    async def generate_attack(
        ...
        phrases_to_try = (
            self.SKELETON_KEY_PHRASES
            if self.use_all_variants
            else [self.SKELETON_KEY_PHRASES[0]]
        )
