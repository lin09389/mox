            ...
            if importance:
                ...
                for i in importance:
                    if max_entropy > min_entropy:
                        importance[i] = 1 - (importance[i] - min_entropy) / (max_entropy - min_entropy)
                    else:
                        importance[i] = 0.5
...
class AdaptiveSearchController:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "improvement_trend": sum(self._improvement_history[-10:]) if self._improvement_history else 0,
...
class ImprovedGCGAttack(BaseAttack):
    ...
    def __init__(
        ...
        self.gradient_optimizer = GradientOptimizer(
            self.gcg_config.gradient_model
        ) if self.gcg_config.use_gradient_guidance else None
        
        self.diversity_selector = SemanticDiversitySelector(
            self.gcg_config.embedding_model
        ) if self.gcg_config.use_semantic_diversity else None
        
        self.adaptive_controller = AdaptiveSearchController(
            self.gcg_config.adaptive_window
        ) if self.gcg_config.use_adaptive_search else None
        
        # 评估器
        self.evaluator = AttackEvaluator(EvaluationConfig(
    ...
    async def generate_attack(
        ...
        # 返回最终结果
        result = AttackResult.SUCCESS if self._best_score >= self.config.success_threshold else AttackResult.FAILURE
    ...
    async def _generate_candidates(self, prompt: str) -> List[Candidate]:
        ...
        while len(candidates) < self.gcg_config.top_k:
            parent = random.choice(candidates) if candidates else Candidate(suffix=self._best_suffix)
            ...
            # 交叉
            if random.random() < self.gcg_config.crossover_rate and len(self._elite_candidates) >= 2:
